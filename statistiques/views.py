...
class Voir_stat(APIView):
    def get(self,request):      
        ...
        try:
            ...
            certificats =certificats_par_date_et_sexe = CertificatNaissance.objects.values('date_deliv_cert', 'sexe_enfant').annotate(count=Count('id'))
...
class Voir_stat_par_hop(APIView):
    ...
    def get(self,request,id):
        ...
        try:
            ...
            certificats = CertificatNaissance.objects.values('date_deliv_cert', 'sexe_enfant', 'hospital_id').filter(hospital_id=id).annotate(count=Count('id'))
        except CertificatNaissance.DoesNotExist:
            return Response({"message":"l'hopital n'a pas de certoificats enregistrer"})
            # Triez les certificats par date
        certificats_tries = sorted(certificats, key=lambda x: x['date_deliv_cert'])
